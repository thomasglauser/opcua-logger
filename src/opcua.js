...
async function _connectUA(endpointUrl, userName, password) {
    ...
    UASESSION = userName
        ? await UACLIENT.createSession({ userName, password })
        : await UACLIENT.createSession();
    log.info('Established session with server.', {
        ID: UASESSION.sessionId.value,
    });
    ...
    UASUBSCRIPTION.on('started', () => {
        log.info('Installed subscription on session.', {
            ID: UASUBSCRIPTION.subscriptionId,
        });
    })
        .on('keepalive', () => {})
        .on('error', () => {
...
function _addPolledMetric(metric) {
    let pi =
        metric.interval / 1000 > 0 || 5000
            ? Math.min(Math.trunc(metric.interval / 1000), 60)
            : 5;
    while (60 % pi !== 0) {
        pi++;
    }
...
function _addMonitoredMetric(metric) {
    ...
    uaMonitoredItem
        ...
        .on('err', (err) => {
            log.error('MonitoredItem returned error.', {
                nodeId: metric.nodeId,
                error: err,
            });
